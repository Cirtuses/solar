...
dst_path = os.path.join('/media/nucleus/solar/data_set', str(delay_time) + 's-' + str(interval_time) + 's', DNI,
                        LR_or_HR)
train_data_path = '/media/nucleus/solar/TongJi5F-LRHR/train_summer'
test_data_path = '/media/nucleus/solar/TongJi5F-LRHR/test_sim'
csv_file_path = '/media/nucleus/solar/data_set' + '/' + str(delay_time)  + 's-' + str(
    interval_time) + 's' + "/" + DNI + "/"  #数据集命名 horzion + interval
csv_filename = csv_file_path + "data_" + str(delay_time)  + 'min_' + LR_or_HR + ".csv"
...
def excute(path, index, flag):
    data = read_csv(path)
    # path1 = os.path.join(path, LR_or_HR + '-HDR/')
    path2 = os.path.join(path, LR_or_HR)
